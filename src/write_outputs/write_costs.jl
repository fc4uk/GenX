...
function write_costs(path::AbstractString, inputs::Dict, setup::Dict, EP::Model)
	...
	dfCost = DataFrame(Costs = ["cTotal", "cFix", "cVOM", "cFuel", "cNSE", "cStart", "cUnmetRsv", "cNetworkExp"])
	...
	dfCost[!,Symbol("Total")] = [objective_value(EP), cFix, cVOM, cFuel, value(EP[:eTotalCNSE]), 0.0, 0.0, 0.0]
	...
	for z in 1:Z
		...
		eCVOM = sum(value.(EP[:ePlantCVOMOut][Y_ZONE]))
		tempCVOM += eCVOM
		tempCTotal += tempCVOM 

		eCFuel = sum(value.(EP[:ePlantCFuelOut][Y_ZONE]))
		tempCFuel += eCFuel
		...
		dfCost[!,Symbol("Zone$z")] = [tempCTotal, tempCFix, tempCVOM, tempCFuel, tempCNSE, tempCStart, "-", "-"]
	end
