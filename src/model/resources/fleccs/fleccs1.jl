...
@doc raw"""
	...
	The FLECCS1 module creates decision variables, expressions, and constraints related to conventional NGCC-CCS, the system is breakdown into gas turbine, steam turbine, post-combustion carbon capture, CO2 compressor, and balance of plant (BOP)
"""
...
function fleccs1(EP::Model, inputs::Dict)
	...
	@variables(EP, begin
        vCAPTURE[y in FLECCS_ALL,1:T] >= 0 # captured CO2, t CO2/h
	end)
	...
	# Thermal Energy of steam generated by HRSG at hour "t" , high pressure steam [MMBTU/h] = Steam generation rate, high pressure [fraction] * energy content of fuel [MMBTU]
	@expression(EP, eSteam_high[y in FLECCS_ALL,t=1:T], dfGen_ccs[!,:pSteamRate_high][1+n*(y-1)] * eFuel[y,t])
	# mid pressure steam [MMBTU/h]
	@expression(EP, eSteam_mid[y in FLECCS_ALL,t=1:T], dfGen_ccs[!,:pSteamRate_mid][1+n*(y-1)] * eSteam_high[y,t])
	# low pressure steam [MMBTU/h]
	...
	#steam used by post-combustion carbon capture (PCC) unit [MMBTU/h] = steam use rate [MMBTU/t CO2] * captured CO2 [tonne/h]
	...
	#mass of compressed CO2 [tonne CO2/h] = captured CO2 [tonne CO2/h]
	...
	#Power generated by high pressure steam turbine [MW] = energy content of high pressure steam [MMBTU/h] * heat rate of high pressure steam turbine [MWh/MMBTU]
	@expression(EP, ePower_st_high[y in FLECCS_ALL,t=1:T], eSteam_high[y,t]/dfGen_ccs[!,:pHeatRate_st_high][1+n*(y-1)] )

	#Power generated by mid pressure steam turbine [MW] = energy content of mid pressure steam [MMBTU/h] * heat rate of mid pressure steam turbine [MWh/MMBTU]
	@expression(EP, ePower_st_mid[y in FLECCS_ALL,t=1:T], eSteam_mid[y,t]/dfGen_ccs[!,:pHeatRate_st_mid][1+n*(y-1)])

	#Power generated by high pressure steam turbine [MW] =(energy content of high pressure steam [MMBTU/h] - energy used for PCC [MMBTU/h]) * heat rate of low pressure steam turbine [MWh/MMBTU]
	@expression(EP, ePower_st_low[y in FLECCS_ALL,t=1:T], (eSteam_low[y,t]- eSteam_use_pcc[y,t])/dfGen_ccs[!,:pHeatRate_st_low][1+n*(y-1)])
    
	# ePower_st_low must be non-negative
	@constraint(EP, [y in FLECCS_ALL,t=1:T], ePower_st_low[y,t] >= 0 )

	#Power generated by steam turbine [MW] = power output from high pressure steam turbine [MW] + power output from mid pressure steam turbine [MW] +power output from low pressure steam turbine [MW] 
	@expression(EP, ePower_st[y in FLECCS_ALL,t=1:T], ePower_st_high[y,t] + ePower_st_mid[y,t] + ePower_st_mid[y,t])

	# NGCC-CCS net power output = vP_gt  + ePower_st - ePower_use_comp - ePower_use_pcc - ePower_use_other
	...
	# add the net power output from ngcc-ccs to the power balance equation per zone
	...
	# create a container for FLECCS output so they can be passed to fleccs_commit and fleccs_no_commit modules to enforce the unit commitment
	...
	# amount of CO2 sequestration [tonne/h]
	@expression(EP, eCO2_sequestration[y in FLECCS_ALL,t=1:T], vCAPTURE[y,t])


	########### variable cost
	#fuel costs [$/h] = fuel consumption [MMBTU/h] * fuel cost [$/MMBTU] 
	...
	@expression(EP,eCVOMFLECCS[t = 1:T], sum(eCVar_CO2_sequestration[y,t] + eCVar_gt[y,t] + eCVar_st[y,t] + eCVar_comp[y,t] + eCVar_pcc[y,t]  for  t in 1:T))
	# Sum to zonal-annual level
